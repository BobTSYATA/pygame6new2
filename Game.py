...
def main(player1_type, player2_type):
    ...
    while not testing_done:
        ...
        while not done:
            ...
            # Player 1's turn
            if agent_type1 == "DQN":
                action_tuple_1 = player_1.get_Action(environment, state=state, train=False, epoch=1)
            elif agent_type1 == "Random_Agent" or agent_type1 == "Human_Agent":
                action_tuple_1 = player_1.get_Action(environment, "1", events=events, state=state)
